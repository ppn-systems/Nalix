...
The `Nalix.Common` package defines the fundamental interfaces, attributes, and data structures that are shared across the entire Nalix stack, including both the server-side runtime and the client-side SDK.

## Core Contracts

- [**Packet Contracts**](./packet-contracts.md) — `IPacket`, `ProtocolOpCode`, and basic framing.
...
- [**Diagnostics Contracts**](./diagnostics-contracts.md) — `IConnectionErrorTracked` and metrics.
