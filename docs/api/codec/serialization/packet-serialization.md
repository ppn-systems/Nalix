...
When no explicit formatter is registered, Nalix relies on **Source Generation** to create optimized formatters at compile-time for:

- types marked with `[GenerateFormatter]` or `[Packet]`
- types used as members within other source-generated models

This avoids the overhead of reflection and IL Emit, providing full AOT compatibility and ultra-high performance.
