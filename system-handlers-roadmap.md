...
- **Architectural Guidelines:**
  ...
  - **Dedicated Session Manager (`ISessionManager`):** Establish an independent module to exclusively govern the lifecycle of `SessionSnapshot` records. This manager provides a unified abstraction over internal `MemoryCache` or distributed `Redis` instances, handling token generation, secure storage, and strict TTL expiration (e.g., 5-minute automatic eviction).
  - **Caching Strategy (TCP Half-Open Mitigation):** The `SessionSnapshot` MUST be generated by the Session Manager and committed to the Cache **immediately** upon a successful Handshake sequence. Waiting for the socket `Dispose` event is a fatal anti-pattern since dead mobile connections (TCP Half-Open without FIN flags) may take 30+ seconds to trigger a disconnect, breaking the ultra-fast reconnect flow if the cache isn't pre-warmed.
  - **Hydration:** Validate the token via the `ISessionManager` to retrieve the active `IConnection` cipher state.
