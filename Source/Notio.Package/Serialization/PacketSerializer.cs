...
internal static class PacketSerializer
{
    ...
    internal static Packet ReadPacketFast(ReadOnlySpan<byte> data)
    {
        ...
        if ((uint)length > data.Length)
            throw new
                PackageException($"Invalid packet length: {length}. Must be between {PacketSize.Header} and {data.Length}.");
        ...
        (
            ...
            priority: Unsafe.ReadUnaligned<byte>(ref Unsafe.Add(ref dataRef, PacketOffset.Priority))
        );
    }
}