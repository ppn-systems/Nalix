...
public class CapturedMultipartContent : MultipartContent
{
    ...
    private readonly List<HttpContent> _capturedParts = [];
    ...
    /// </summary>
    /// <param name="settings">The HttpSettings used to serialize each content part.</param>
    public CapturedMultipartContent(HttpSettings settings) : base("form-data")
    {
        _settings = settings;
    }
    ...
    /// <param name="settings">The HttpSettings used to serialize each content part.</param>
    public CapturedMultipartContent(string subtype, HttpSettings settings) : base(subtype)
    {
        _settings = settings;
    }
    ...
    /// <param name="settings">The HttpSettings used to serialize each content part.</param>
    public CapturedMultipartContent(string subtype, string boundary, HttpSettings settings) : base(subtype, boundary)
    {
        _settings = settings;
    }
    ...
    {
        foreach (var (Key, Value) in data.ToKeyValuePairs())
        {
            if (Value == null)
                continue;
            AddString(Key, Value.ToInvariantString(), contentType);
        }
        return this;
    }
