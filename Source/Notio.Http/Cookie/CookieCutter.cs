...
public static class CookieCutter
{
    ...
    /// <summary>
    /// Parses a Set-Cookie response header to a NotioCookie.
    /// </summary>
    ...
    /// <returns></returns>
    public static NotioCookie ParseResponseHeader(string headerValue, string url, DateTimeOffset? dateReceived = null)
    {
        if (string.IsNullOrEmpty(headerValue)) return null;

        NotioCookie cookie = null;
        foreach (var pair in GetPairs(headerValue))
        {
            if (cookie == null)
                cookie = new NotioCookie(pair.Name, pair.Value.Trim('"'), url, dateReceived ?? DateTimeOffset.UtcNow);
    ...
    /// <summary>
    /// Creates a Set-Cookie response header value from a NotioCookie.
    /// </summary>
    /// <returns>A header value if cookie is non-null, otherwise null.</returns>
    public static string BuildResponseHeader(NotioCookie cookie)
    {
    ...
    /// </summary>
    public static bool IsValid(this NotioCookie cookie, out string reason)
    {
    ...
    /// </summary>
    public static bool IsExpired(this NotioCookie cookie, out string reason)
    {
    ...
    /// </summary>
    public static bool ShouldSendTo(this NotioCookie cookie, Url requestUrl, out string reason)
    {
        ...
        return
            ...
            cookie.IsDomainMatch(requestUrl, out reason) &&
            cookie.IsPathMatch(requestUrl, out reason);
    }

    private static bool IsDomainMatch(this NotioCookie cookie, Url requestUrl, out string reason)
    {
        reason = "ok";
    ...
    private static bool IsPathMatch(this NotioCookie cookie, Url requestUrl, out string reason)
    {
        reason = "ok";
