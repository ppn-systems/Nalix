...
public class CookieSession : IDisposable
{
    ...
    private readonly INotioClient _client;
    ...
    /// <summary>
    /// Creates a new CookieSession where all requests are made using the provided INotioClient
    /// </summary>
    public CookieSession(INotioClient client)
    {
        _client = client;
    }
    ...
    /// <summary>
    /// Creates a new IRequest with this session's CookieJar that can be further built and sent fluently.
    /// </summary>
    /// <param name="urlSegments">The URL or URL segments for the request.</param>
    public IRequest Request(params object[] urlSegments) => _client == null ?
        new Request(_baseUrl, urlSegments).WithCookies(Cookies) :
        new Request(_client, urlSegments).WithCookies(Cookies);
