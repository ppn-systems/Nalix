...
/// </summary>
public class CookieJar : IReadOnlyCollection<NotioCookie>
{
    private readonly ConcurrentDictionary<string, NotioCookie> _dict = new ConcurrentDictionary<string, NotioCookie>();
    ...
    public CookieJar AddOrReplace(string name, object value, string originUrl, DateTimeOffset? dateReceived = null) =>
        AddOrReplace(new NotioCookie(name, value.ToInvariantString(), originUrl, dateReceived));

    /// <summary>
    /// Adds a cookie to the jar or replaces one with the same Name/Domain/Path.
    /// Throws InvalidCookieException if cookie is invalid.
    /// </summary>
    public CookieJar AddOrReplace(NotioCookie cookie)
    {
    ...
    public bool TryAddOrReplace(NotioCookie cookie, out string reason)
    {
    ...
    /// </summary>
    public CookieJar Remove(Func<NotioCookie, bool> predicate)
    {
    ...
    public static CookieJar LoadFrom(TextReader reader)
    {
        ...
        while (reader.Peek() >= 0)
        {
            var cookie = NotioCookie.LoadFrom(reader);
            if (cookie != null)
    ...
    /// <inheritdoc/>
    public IEnumerator<NotioCookie> GetEnumerator() => _dict.Values.GetEnumerator();
...
/// <summary>
/// Exception thrown when attempting to add or update an invalid NotioCookie to a CookieJar.
/// </summary>
