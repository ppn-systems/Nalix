...
public class Url
{
    ...
    public Url(string baseUrl = null) => _originalString = baseUrl?.Trim();
    ...
    private Url ParseInternal(Uri uri = null)
    {
        ...
        else if (uri.IsAbsoluteUri)
        {
            ...
            _host = uri.Host;
            _port = _originalString?.OrdinalStartsWith($"{Root}:{uri.Port}", ignoreCase: true) == true ? uri.Port : null; // don't default Port if not included explicitly
            _pathSegments = [];
    ...
    public Uri ToUri() => new(this, UriKind.RelativeOrAbsolute);
    ...
    public static string EncodeIllegalCharacters(string s, bool encodeSpaceAsPlus = false)
    {
        ...
        {
            var a = c.Groups[1].Value;
            var b = c.Groups[2].Value;
