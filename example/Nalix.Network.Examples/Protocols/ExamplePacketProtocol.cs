...
public sealed class ExamplePacketProtocol : Protocol
{
    ...
    /// </summary>
    protected override void OnConnectionError(IConnection connection, Exception exception) => base.OnConnectionError(connection, exception);//InstanceManager.Instance.GetExistingInstance<ILogger>()?//                        .Error($"[ExamplePacketProtocol] Connection {connection.ID} failed.", exception);
}
