...
internal sealed class PingChart
{
    ...
    public void AddSample(double ms)
    {
        ...
        {
            _samples.Enqueue(ms);
            if (_samples.Count > MaxSamples) { _ = _samples.Dequeue(); }
            if (ms < _min)
            {
                _min = ms;
            }
    ...
    public void Render()
    {
        ...
        if (snap.Length == 0)
        {
            Panel empty = new("[grey italic]No ping samples yet. Run [mediumpurple1]Continuous Ping[/] to populate.[/]")
            {
