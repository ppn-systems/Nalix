...
public static class Crc32
{
    ...
    {
        return HashToUInt32(data) == expectedCrc;
    }
    ...
    {
        return HashToUInt32(MemoryMarshal.AsBytes(data));
    }
    ...
    private static uint ProcessOctet(uint crc, ReadOnlySpan<byte> octet)
    {
        ...
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[0]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[1]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[2]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[3]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[4]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[5]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[6]];
        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ octet[7]];
        ...
        if (vectorCount > 0)
        {
            fixed (byte* ptr = bytes)
            {
                for (int i = 0; i < vectorCount * vectorSize; i += vectorSize)
                {
                    ...
                    {
                        crc = (crc >> 8) ^ Table[(crc & 0xFF) ^ ptr[i + j]];
                    }
                }
