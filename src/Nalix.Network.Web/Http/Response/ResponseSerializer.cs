...
{
    /// <summary>
    /// Gets the JSON serializer options configured for HTTP responses.
    /// </summary>
    ...
    public static async Task Json(IHttpContext context, object? data)
    {
        ...
        await ChunkedEncodingBaseSerializer(context, JsonSerializer.Serialize(data, HttpFormatted))
        ...
        return async (context, data) =>
            {
                ...
                await ChunkedEncodingBaseSerializer(
                    context, JsonSerializer.Serialize(data, HttpFormatted))
                    .ConfigureAwait(false);
            };
    }
    ...
    public static ResponseSerializerCallback Json(bool bufferResponse)
        => async (context, data) =>
        {
            ...
            await baseSerializer(context, JsonSerializer.Serialize(data, HttpFormatted))
                .ConfigureAwait(false);
        };
