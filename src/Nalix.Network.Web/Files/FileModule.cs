...
using Nalix.Network.Web.Files.Internal;
...
public class FileModule : WebModuleBase, IDisposable, IMimeTypeCustomizer
{
    ...
    protected override async Task OnRequestAsync(IHttpContext context)
    {
        ...
        else if (!IsHttpMethodAllowed(context.Request, out bool sendResponseBody))
        {
    ...
    private MappedResourceInfo? MapUrlPath(string urlPath, IMimeTypeProvider mimeTypeProvider)
    {
        ...
        {
            string defaultDocumentPath = urlPath + (urlPath.Length > 1 ? "/" : string.Empty) + DefaultDocument;
    ...
    private async Task HandleResource(IHttpContext context, MappedResourceInfo info, bool sendResponseBody)
    {
        ...
        long cachingThreshold = 1024L * Cache.MaxFileSizeKb;
        ...
        string contentType = info.ContentType ?? DirectoryLister?.ContentType ?? MimeType.Default;
        string mimeType = MimeType.StripParameters(contentType);
        ...
        if (!context.TryDetermineCompression(mimeType, out bool preferCompression))
        {
        ...
        string entityTag = info.GetEntityTag(compressionMethod);
        ...
        long contentLength = content?.Length ?? info.Length;
        ...
        long partialUpperBound = contentLength - 1;
        bool isPartial = compressionMethod == CompressionMethod.None
                     && context.Request.IsRangeRequest(contentLength, entityTag, info.LastModifiedUtc, out partialStart, out partialUpperBound);
        long responseContentLength = contentLength;
        ...
        if (content == null && ContentCaching && contentLength <= cachingThreshold)
        {
            ...
            {
                using (CompressionStream compressor = new(memoryStream, compressionMethod))
                {
        ...
        using (Stream source = Provider.OpenFile(info.Path))
        {
            ...
            if (isPartial)
            {
                byte[] buffer = new byte[WebServer.StreamCopyBufferSize];
                if (source.CanSeek)
                {
            ...
            {
                using CompressionStream compressor = new(context.Response.OutputStream, compressionMethod);
                await source.CopyToAsync(compressor, WebServer.StreamCopyBufferSize, context.CancellationToken)
                    .ConfigureAwait(false);
            }
        }
    ...
    {
        using MemoryStream memoryStream = new();
        using CompressionStream stream = new(memoryStream, compressionMethod);
