...
public sealed class RouteMatcher : IEquatable<RouteMatcher>
{
    ...
    private static readonly Dictionary<(bool, string), RouteMatcher> Cache = [];
    ...
    {
        return TryParseInternal(route, isBaseRoute, out result) == null;
    }
    ...
    {
        return obj is RouteMatcher other && Equals(other);
    }
    ...
    public RouteMatch? Match(string path)
    {
        ...
        return !match.Success
            ...
            IsBaseRoute ? "/" + path[match.Groups[0].Length..] : null);
    }
