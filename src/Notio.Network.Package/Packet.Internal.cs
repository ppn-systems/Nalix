...
public readonly partial struct Packet
{
    ...
    internal Packet(
        ushort id, PacketCode code, PacketType type, PacketFlags flags,
        PacketPriority priority, byte number, System.Memory<byte> payload)
        : this(id, 0, Notio.Utilities.MicrosecondClock.GetTimestamp(),
    ...
    [System.Runtime.CompilerServices.MethodImpl(
            System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
    internal Packet(ushort id, uint checksum, ulong timestamp,
        ushort code, byte type, byte flags, byte priority,
        byte number, System.Memory<byte> payload, bool computeChecksum = false)
        : this(id, checksum, timestamp, (PacketCode)code, (PacketType)type,
              (PacketFlags)flags, (PacketPriority)priority, number, payload, computeChecksum)
    {
    }
    ...
    [System.Runtime.CompilerServices.MethodImpl(
            System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
    internal Packet(ushort id, uint checksum, ulong timestamp,
        PacketCode code, PacketType type, PacketFlags flags, PacketPriority priority,
        byte number, System.Memory<byte> payload, bool computeChecksum = false)
    {
        ...
        Payload = Notio.Utilities.MemoryAllocator.Allocate(payload);
