...
public static class JsonBinary
{
    ...
    /// <summary>
    /// Serializes an object to a JSON byte array (UTF-8 encoded) using a Memory<byte>.
    /// </summary>
    /// <typeparam name="T">The type of the object to serialize.</typeparam>
    /// <param name="obj">The object to serialize.</param>
    /// <param name="jsonTypeInfo">The metadata used for JSON serialization. Must not be <see langword="null"/>.</param>
    /// <returns>A Memory<byte> containing the UTF-8 encoded JSON representation of the object.</returns>
    /// <exception cref="ArgumentNullException">Thrown if <paramref name="jsonTypeInfo"/> is <see langword="null"/>.</exception>
