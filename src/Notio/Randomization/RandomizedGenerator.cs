...
public static class RandomizedGenerator
{
    ...
    public static void Fill(Span<byte> data)
    {
        ...
        {
            BitConverter.TryWriteBytes(data.Slice(i, 8), NextUInt64());
            i += 8;
            remainingBytes -= 8;
        }
    ...
    public static byte[] DeriveKey(string passphrase, int length = 32, int iterations = 100000)
    {
        ...
        // Get thread-local buffer for passphrase bytes
        ...
        bool includeLowercase = true)
    {
        ...
        if (includeLowercase)
        {
            password.Append(lowercaseChars[(int)(NextUInt64() % (uint)lowercaseChars.Length)]);
            requiredCharCount++;
        }

        if (includeUppercase && requiredCharCount < length)
        {
            password.Append(uppercaseChars[(int)(NextUInt64() % (uint)uppercaseChars.Length)]);
            requiredCharCount++;
        }

        if (includeNumbers && requiredCharCount < length)
        {
            password.Append(numberChars[(int)(NextUInt64() % (uint)numberChars.Length)]);
            requiredCharCount++;
        }

        if (includeSpecial && requiredCharCount < length)
        {
            password.Append(specialChars[(int)(NextUInt64() % (uint)specialChars.Length)]);
            requiredCharCount++;
            password.Append(specialChars[(int)(NextUInt64() % (ulong)specialChars.Length)]);
        }
        ...
        for (int i = requiredCharCount; i < length; i++)
        {
    ...
    private static void InitializeState()
    {
        ...
        // Use DateTime.UtcNow.Ticks as part of the seed
        long ticks = DateTime.UtcNow.Ticks;
        BitConverter.TryWriteBytes(seed.AsSpan(0, 8), ticks);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void FillBlock(Span<byte> block)
    {
        // Generate four 64-bit values in sequence
        BitConverter.TryWriteBytes(block[0..8], NextUInt64());
        BitConverter.TryWriteBytes(block[8..16], NextUInt64());
        BitConverter.TryWriteBytes(block[16..24], NextUInt64());
        BitConverter.TryWriteBytes(block[24..32], NextUInt64());
    }
}
