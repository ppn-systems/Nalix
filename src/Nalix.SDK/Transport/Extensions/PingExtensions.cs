...
public static class PingExtensions
{
    ...
    public static async ValueTask<double> PingAsync(this TransportSession session, int timeoutMs = 5000, CancellationToken ct = default)
    {
        ...
        TimeSync ping = session.NewTimeSync((ushort)ProtocolOpCode.SYSTEM_CONTROL, ControlType.PING).WithSeq(seq).Build();
        try
        {
