    ...
    private void Initialize()
    {
        if (_config.EnableIPv6)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogDebug("[NW.TcpListenerBase:Initialize] config-bind {EpV6Any}.v6)", epV6Any);
                }
                ...
                if (_logger != null && _logger.IsEnabled(LogLevel.Debug))
                {
                    _logger.LogDebug("[NW.TcpListenerBase:Initialize] config-listen {LocalEndpoint}.dual", _listener.LocalEndPoint);
                }
            ...
            catch (Exception ex) when (ExceptionClassifier.IsNonFatal(ex))
            {
                ...
                {
                    _logger.LogWarning(ex, "[NW.TcpListenerBase:Initialize] failed-bind");
                }
                ...
                catch (ObjectDisposedException closeEx)
                {
                    if (_logger != null && _logger.IsEnabled(LogLevel.Debug))
                    {
                        _logger.LogDebug(ex, "[NW.TcpListenerBase:Initialize] ipv6-fallback-close-ignored reason={Type}", closeEx.GetType().Name);
                    }
                }
                catch (Exception closeEx) when (ExceptionClassifier.IsNonFatal(closeEx))
                {
                    if (_logger != null && _logger.IsEnabled(LogLevel.Warning))
                    {
                        _logger.LogWarning(closeEx, "[NW.TcpListenerBase:Initialize] ipv6-fallback-close-failed");
                    }
                }
                ...
                catch (Exception disposeEx) when (ExceptionClassifier.IsNonFatal(disposeEx))
                {
                    if (_logger != null && _logger.IsEnabled(LogLevel.Warning))
                    {
                        _logger.LogWarning(disposeEx, "[NW.TcpListenerBase:Initialize] ipv6-fallback-dispose-failed");
                    }
                }
            }
        }
        ...
        if (_logger != null && _logger.IsEnabled(LogLevel.Debug))
        {
            _logger.LogDebug("[NW.TcpListenerBase:Initialize] config-bind {EpV4Any}.v4", epV4Any);
        }
        ...
        if (_logger != null && _logger.IsEnabled(LogLevel.Debug))
        {
            _logger.LogDebug("[NW.TcpListenerBase:Initialize] config-listen {LocalEndpoint}", _listener.LocalEndPoint);
        }
    }
    ...
    protected void InitializeOptions(Socket socket)
    {
        ...
        if (_config.ReusePort)
        {
            ...
            catch (SocketException ex) when (ex.SocketErrorCode is SocketError.OperationNotSupported or SocketError.ProtocolNotSupported)
            {
                ...
                {
                    _logger.LogDebug("[TcpListenerBase:InitializeOptions] SO_REUSEPORT not-supported platform/kernel");
                }
            }
