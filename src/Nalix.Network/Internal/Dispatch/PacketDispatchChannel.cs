    ...
    : PacketDispatchCore<IPacket>, IPacketDispatch<IPacket>, System.IDisposable, IActivatable
{
    ...
    public PacketDispatchChannel(System.Action<PacketDispatchOptions<IPacket>> options)
        : base(options)
    {
    ...
    public void HandlePacket(System.Byte[]? raw, IConnection connection)
    {
        if (raw == null)
        {
            Logger?.Warn($"[{nameof(PacketDispatch)}] NONE byte[] received from {connection.RemoteEndPoint}. Packet dropped.");
            return;
        }
