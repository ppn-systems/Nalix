...
internal sealed class SocketTcpTransport(Connection outer) : IConnection.ITcp
{
    ...
    [StackTraceHidden]
    [MethodImpl(MethodImplOptions.NoInlining)]
    [Obsolete("This method may produce multiple packets for large messages. Consider using a different approach for large data transmission.")]
        ...
        foreach (Candidate c in UTF8_STRING.Candidates)
        {
            if (byteCount <= c.MaxBytes)
            {
                ...
                try
                {
                    ...
                    byte[] buffer = c.Serialize(pkt);
                    _outer.AddBytesSent(buffer.Length);
                    await this.SendAsync(buffer, cancellationToken).ConfigureAwait(false);
                    return;
                }
                finally { c.Return(pkt); }
            }
