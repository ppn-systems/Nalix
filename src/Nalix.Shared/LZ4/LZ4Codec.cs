...
namespace Nalix.Shared.LZ4
{
    internal unsafe class LZ4Codec
    {
        ...
        public static int Compress(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            // Allocate hash table on the stack.
            // WARNING: This can cause StackOverflowException for large inputs if HashTableSize is too big!
            if (Matcher.MaxStackallocHashTableSize > 1024 * 1024) // Safety check > 1MB stackalloc
            {
                Console.Error.WriteLine("Warning: HashTable exceeds 1MB, stackalloc may fail.");
            }

            int* hashTable = stackalloc int[Matcher.HashTableSize]; // ~256KB for 64k entries

            // Clear the hash table (important!)
            new Span<byte>(hashTable, Matcher.MaxStackallocHashTableSize).Clear();

            // Slice output buffer to exclude header space initially
            Span<byte> compressedDataOutput = output.Slice(Header.Size);

            int compressedDataLength = FastPath.EncodeBlock(input, compressedDataOutput, hashTable);

            if (compressedDataLength < 0)
            {
                return -1; // Compression failed (likely output buffer too small)
            }
        ...
        public static int Decompress(ReadOnlySpan<byte> input, Span<byte> output)
        {
            ...
            fixed (byte* outputBase = &MemoryMarshal.GetReference(output))
            {
                ...
                while (inputPtr < inputEnd)
                {
                    ...
                    matchLength += Constants.MinMatchLength; // Length is stored relative to min match length
                ...
                {
                    return -1; // Decompression produced wrong size or didn't consume all input
                }
